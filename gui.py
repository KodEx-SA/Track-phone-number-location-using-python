...
def save_history(number, location, detailed_location, service_provider, time_zone, lat, lng, number_type_str):
    history.append({
        "number": number,
        "country": location,
...
def export_to_csv(number, location, detailed_location, service_provider, time_zone, lat, lng, number_type_str):
    filename = "phone_lookup_export.csv"
    file_exists = os.path.isfile(filename)
    with open(filename, "a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["Phone Number", "Country", "Detailed Location", "Service Provider", "Time Zone", "Latitude", "Longitude", "Number Type", "Timestamp"])
        writer.writerow([number, location, detailed_location if detailed_location else "N/A", 
                        ...
                        lat, lng, number_type_str, datetime.now().isoformat()])
...
def track_number():
    ...
    try:
        ...
        save_history(number, location, detailed_location, service_provider, time_zone, lat, lng, number_type_str)
        export_to_csv(number, location, detailed_location, service_provider, time_zone, lat, lng, number_type_str)
...
ttk.Label(input_frame, text="Enter Phone Number (e.g., +1234567890):").pack(anchor="w")
entry = ttk.Entry(input_frame, width=40)
entry.pack(pady=5)
entry.bind("<KeyRelease>", validate_phone_number)

# Map style frame
map_style_frame = ttk.LabelFrame(main_frame, text="Map Style", padding=10)
map_style_frame.pack(fill="x", pady=5)
map_style = tk.StringVar(value="standard")
ttk.Radiobutton(map_style_frame, text="Standard Map", variable=map_style, value="standard", bootstyle="primary").pack(anchor="w")
ttk.Radiobutton(map_style_frame, text="Satellite View", variable=map_style, value="satellite", bootstyle="primary").pack(anchor="w")
ttk.Radiobutton(map_style_frame, text="Terrain View", variable=map_style, value="terrain", bootstyle="primary").pack(anchor="w")
...
result_text = tk.Text(results_frame, height=10, width=50, font=("Helvetica", 10))
